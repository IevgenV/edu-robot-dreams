...
default_config_path = "../configs/main.yml"
...
def test_ProdServer_incorrect_configuration(def_server):
    ...
    with pytest.raises(FileNotFoundError):
        ProdServer("dummy/config/path.yml")

def test_get_out_of_stock_by_date_correct(def_server):
    """ Test of ProdServer::get_out_of_stock_by_date() method correct call. """
    products = def_server.get_out_of_stock_by_date(
        date(2021, 1, 2),
        date(2021, 1, 3))
    assert isinstance(products, dict)
    assert len(products) == 2

def test_get_out_of_stock_by_date_wrong_date_order(def_server):
    """ Test of ProdServer::get_out_of_stock_by_date() method with incorrectly ordered dates. """
    with pytest.raises(AssertionError):
        products = def_server.get_out_of_stock_by_date(
            date(2021, 1, 3),
            date(2021, 1, 2))

def test_get_out_of_stock_today_correct(def_server):
    """ Test of ProdServer::get_out_of_stock_today() method correct call. """
    today_date = date.today()
    products1 = def_server.get_out_of_stock_today()
    products2 = def_server.get_out_of_stock_by_date(today_date, today_date)
...
def test_get_out_of_stock_today_emulate_expired_token(def_server):
    prods = def_server.get_out_of_stock_today()
    assert len(prods) == 1
    def_server.token = expired_token  # <- Faking token with expired/bad one
    with pytest.raises(requests.HTTPError):
        def_server.get_out_of_stock_today()